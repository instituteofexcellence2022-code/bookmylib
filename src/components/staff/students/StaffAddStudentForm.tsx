...
import { createStudent, getStaffBranchOptions } from '@/actions/staff/students'
...
interface StaffAddStudentFormProps {
    onSuccess?: (studentId: string) => void
    onCancel?: () => void
}

export function StaffAddStudentForm({ onSuccess, onCancel }: StaffAddStudentFormProps) {
        ...
        async function loadBranches() {
            try {
                ...
                if (branches.length === 1) {
                    setFormData(prev => ({ ...prev, branchId: branches[0].value }))
                }
            } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-8">
            {/* Basic Info Section */}
                        ...
                        <div className="relative border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors text-center cursor-pointer">
                            ...
                            <div className="flex flex-col items-center gap-2 text-gray-500">
                                ...
                                <span className="text-xs">
                                    {files.image ? files.image.name : 'Click to upload photo'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="relative border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors text-center cursor-pointer">
                            ...
                            <div className="flex flex-col items-center gap-2 text-gray-500">
                                ...
                                <span className="text-xs">
                                    {files.govtId ? files.govtId.name : 'Click to upload ID'}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div className="pt-4 flex justify-end gap-3">
                {onCancel && (
                    <button 
                        type="button"
                        onClick={onCancel}
                        className="px-6 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors font-medium"
                    >
                        Cancel
                    </button>
                )}
                <button 
                    type="submit"
                    disabled={loading}
                    className="px-6 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-600/20 transition-all font-medium disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2"
                >
                    {loading ? (
                        <>
