...
import { Seat, StudentSubscription, Student } from '@prisma/client'

// Define types
export type SeatWithRelations = Seat & {
  branch: { name: string }
  subscriptions: (Pick<StudentSubscription, 'id' | 'startDate' | 'endDate'> & {
    student: Pick<Student, 'name' | 'image'>
    plan?: { name: string }
  })[]
}
...
interface SeatManagementProps {
  ...
  actions: {
    ...
    createBulk?: (data: any) => Promise<{ success: boolean; count?: number; skipped?: any[]; error?: string }>
    ...
    getEligibleStudents: (branchId?: string) => Promise<{ success: boolean; data?: any[]; error?: string }>
    assign: (seatId: string, subscriptionId: string) => Promise<{ success: boolean; error?: string }>
    unassign: (subscriptionId: string) => Promise<{ success: boolean; error?: string }>
  }
}

export function SeatManagement({ initialSeats, branches, actions }: SeatManagementProps) {
  const router = useRouter()
  const [seats, setSeats] = useState<SeatWithRelations[]>(initialSeats)
  ...
  const [selectedSeat, setSelectedSeat] = useState<SeatWithRelations | null>(null)
  const [isLoading, setIsLoading] = useState(false)

  // Derived State
  const filteredSeats = useMemo(() => {
    return seats.filter(seat => {
      const matchesBranch = !branches || selectedBranch === 'all' || seat.branchId === selectedBranch
      ...
      const matchesSearch = 
        ...
        (seat.section && seat.section.toLowerCase().includes(searchQuery.toLowerCase())) ||
        seat.subscriptions?.some((sub) => sub.student.name.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  }, [seats, branches, selectedBranch, selectedType, selectedStatus, searchQuery])
  ...
  // Group seats by branch only if we are in owner view (branches prop exists)
  const groupedSeats = useMemo(() => {
    if (!branches) return { 'All Seats': filteredSeats }
    
    return filteredSeats.reduce((acc: Record<string, SeatWithRelations[]>, seat) => {
      const branchName = seat.branch.name
      if (!acc[branchName]) acc[branchName] = []
      acc[branchName].push(seat)
      return acc
    }, {})
  }, [filteredSeats, branches])
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (branches) {
        data.branchId = formData.get('branchId') as string
    }
    ...
    if (result.success) {
      ...
      router.refresh()
      if (result.data) {
        // If we have branches, we need to add the branch name to the result for display
        const branchName = branches 
            ? branches.find(b => b.id === data.branchId)?.name || 'Unknown' 
            : seats[0]?.branch.name || 'Current Branch' // Fallback for staff
            
        setSeats(prev => [...prev, { ...result.data, branch: { name: branchName }, subscriptions: [] }])
      }
    } else {
  ...
  const handleBulkCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await actions.createBulk(data)
    if (result.success) {
  ...
  const handleUpdate = async (id: string, data: any) => {
    ...
    if (result.success && result.data) {
      ...
      setSeats(prev => prev.map(s => s.id === id ? { ...s, ...result.data } : s))
      setSelectedSeat(prev => prev ? { ...prev, ...result.data } : null)
      router.refresh()
      return true
    } else {
      toast.error(result.error)
          ...
          <div key={groupName} className="space-y-4">
            {branches && (
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{groupName}</h3>
                <span className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-500 text-xs rounded-full">
                  {groupSeats.length} seats
                </span>
              </div>
            )}
...
const StatCard = memo(function StatCard({ label, value, icon: Icon, color }: { label: string, value: string | number, icon: any, color: string }) {
  return (
...
const SeatGridItem = memo(function SeatGridItem({ seat, onClick }: { seat: SeatWithRelations, onClick: () => void }) {
  const isOccupied = seat.subscriptions && seat.subscriptions.length > 0
...
const SeatListItem = memo(function SeatListItem({ seat, onClick }: { seat: SeatWithRelations, onClick: () => void }) {
  const isOccupied = seat.subscriptions && seat.subscriptions.length > 0
  const isMaintenance = !seat.isActive
...
function SeatDetailModal({ seat, onClose, onDelete, onUpdate, actions }: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (activeTab === 'history') {
        const res = await actions.getHistory(seat.id)
        if (res.success && res.data) setHistory(res.data)
      } else if (activeTab === 'assign' && !isOccupied && !isMaintenance) {
        const res = await actions.getEligibleStudents(seat.branchId)
  ...
  const handleAssign = async (subscriptionId: string) => {
    ...
    setIsLoading(true)
    const res = await actions.assign(seat.id, subscriptionId)
    if (res.success) {
  ...
  const handleUnassign = async () => {
      ...
      setIsLoading(true)
      const res = await actions.unassign(sub.id)
      if (res.success) {
