...
import { Locker, StudentSubscription, Student, Plan } from '@prisma/client'

// Define strict types
export type LockerWithRelations = Locker & {
  branch: { name: string }
  subscriptions: (Pick<StudentSubscription, 'id' | 'startDate' | 'endDate'> & {
    student: Pick<Student, 'name' | 'image' | 'email'>
    plan: Pick<Plan, 'name'>
  })[]
}
...
interface LockerManagementProps {
  ...
  actions: {
    ...
    assign: (lockerId: string, subscriptionId: string) => Promise<{ success: boolean; error?: string }>
    unassign: (subscriptionId: string) => Promise<{ success: boolean; error?: string }>
  }
}

export function LockerManagement({ initialLockers, branches, actions }: LockerManagementProps) {
  const router = useRouter()
  const [lockers, setLockers] = useState<LockerWithRelations[]>(initialLockers)
  ...
  const [selectedLocker, setSelectedLocker] = useState<LockerWithRelations | null>(null)
  const [isLoading, setIsLoading] = useState(false)

  // Derived State
  const filteredLockers = useMemo(() => {
    return lockers.filter(locker => {
      const matchesBranch = !branches || selectedBranch === 'all' || locker.branchId === selectedBranch
      ...
      const matchesSearch = 
        ...
        locker.subscriptions?.some((sub) => sub.student.name.toLowerCase().includes(searchQuery.toLowerCase()))
      
      return matchesBranch && matchesStatus && matchesSearch
    }).sort((a, b) => {
      return a.number.localeCompare(b.number, undefined, { numeric: true, sensitivity: 'base' })
    })
  }, [lockers, selectedBranch, selectedStatus, searchQuery, branches])
  ...
  const groupedLockers = useMemo(() => {
    return filteredLockers.reduce((acc: Record<string, LockerWithRelations[]>, locker) => {
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.success) {
      ...
      router.refresh()
      if (result.data) {
        // If we have branches, we need to find the branch object to add to the new locker
        let branch = { name: 'Current Branch' }
        if (branches && data.branchId) {
            const foundBranch = branches.find(b => b.id === data.branchId)
            if (foundBranch) branch = foundBranch
        } else if (lockers.length > 0) {
            // Fallback for staff view where branch is implicit
            branch = lockers[0].branch
        }
  ...
  const handleUpdate = async (id: string, data: any) => {
    ...
    if (result.success && result.data) {
      ...
      setLockers(prev => prev.map(l => l.id === id ? { ...l, ...result.data } : l))
      setSelectedLocker(prev => prev ? { ...prev, ...result.data } : null)
      router.refresh()
      return true
    } else {
      toast.error(result.error)
...
const LockerGridItem = memo(function LockerGridItem({ locker, onClick }: { locker: LockerWithRelations, onClick: () => void }) {
...
const LockerListItem = memo(function LockerListItem({ locker, onClick }: { locker: LockerWithRelations, onClick: () => void }) {
  const isOccupied = locker.subscriptions && locker.subscriptions.length > 0
  const isMaintenance = !locker.isActive
...
function LockerDetailModal({ locker, onClose, onDelete, onUpdate, actions }: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (activeTab === 'history') {
        const res = await actions.getHistory(locker.id)
        if (res.success && res.data) setHistory(res.data)
      } else if (activeTab === 'assign' && !isOccupied && !isMaintenance) {
        const res = await actions.getEligibleStudents(locker.branchId)
        if (res.success && res.data) setEligibleStudents(res.data)
      }
      setIsLoadingData(false)
    }
    fetchData()
  }, [activeTab, locker.id, locker.branchId, isOccupied, isMaintenance, actions])
  ...
  const handleAssign = async (subscriptionId: string) => {
    const res = await actions.assign(locker.id, subscriptionId)
    if (res.success) {
  ...
  const handleUnassign = async () => {
    ...
    const res = await actions.unassign(currentSub.id)
    if (res.success) {
