...
interface DashboardData {
  ...
  upcomingExpirations: Array<SubscriptionData> // Kept for type compatibility but might be unused
  attendance: {
    today: number
    totalActive: number
    percentage: number
  ...
  recentlyExpired: Array<SubscriptionData> // Kept for type compatibility
}
...
export default function OwnerDashboard() {
  ...
  const [expiringList, setExpiringList] = useState<SubscriptionData[]>([])
  ...
  const [expiredList, setExpiredList] = useState<SubscriptionData[]>([])
  ...
  // Activity Feed State
  const [activities, setActivities] = useState<DashboardData['recentActivity']>([])
  ...
  const fetchExpiring = useCallback(async (reset = false) => {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            if (reset) setExpiringPage(2) // Next page will be 2
        }
    } catch (error) {
        console.error(error)
  ...
  }, [expiringSoonFilterDays, selectedBranch, isMounted]) // Depend on selectedBranch too

  useEffect(() => {
    if (isMounted) fetchExpired(true)
  }, [expiredFilterDays, selectedBranch, isMounted])
  ...
  const loadMoreActivities = async () => {
    ...
    try {
        ...
        // Ensure time is a string for serialization or Date object handling
