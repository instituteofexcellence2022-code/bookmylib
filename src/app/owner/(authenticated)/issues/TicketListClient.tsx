...
export default function TicketListClient({ 
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    const matchesBranch = branchFilter === 'all' || ticket.student?.branchId === branchFilter
    const matchesCategory = categoryFilter === 'all' || ticket.category === categoryFilter
    const matchesPriority = priorityFilter === 'all' || ticket.priority === priorityFilter
    
    return matchesSearch && matchesStatus && matchesBranch && matchesCategory && matchesPriority
  })
  ...
  const hasActiveFilters = searchTerm !== '' || statusFilter !== 'all' || branchFilter !== 'all' || categoryFilter !== 'all' || priorityFilter !== 'all'
              ...
              {filteredTickets.length === 0 ? (
                ...
                filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                        {ticket.student?.branch ? (
                            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                                <Building className="w-4 h-4" />
                                <span>{ticket.student.branch.name}</span>
                            </div>
                        ) : (
