...
export async function getStaffKhatabook(limit = 50) {
    try {
        ...
        console.error('Error fetching staff khatabook:', error)
        return { success: false, error: 'Failed to fetch staff khatabook' }
    }
}
...
export async function getPendingCashTransactions() {
    try {
        ...
        console.error('Error fetching pending cash transactions:', error)
        return { success: false, error: 'Failed to fetch pending cash transactions' }
    }
}
...
export async function getStaffCashSummary() {
    try {
        ...
        console.error('Error fetching staff cash summary:', error)
        return { success: false, error: 'Failed to fetch staff cash summary' }
    }
}
...
export async function createHandoverRequest(data: {
    ...
    try {
        ...
        // Validate amount
        const summaryResult = await getStaffCashSummary()
        if (!summaryResult.success || !summaryResult.data) {
            return { success: false, error: 'Failed to validate cash summary' }
        }
        ...
        if (data.amount > availableCash) {
            return { success: false, error: 'Insufficient cash in hand (some amount may be pending approval)' }
        }
        ...
        return { success: false, error: 'Failed to create handover request' }
    }
}
